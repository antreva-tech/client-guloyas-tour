...
const NIGHTFALL_GRADIENT = "linear-gradient(180deg, #19173b 0%, #272454 60%, #5a58a2 100%)";

/**
 * WhatsApp icon for contact CTA.
 */
...
/**
 * Contact + social section: one consolidated box (WhatsApp CTA + social follow)
 * separated by a brand-gold divider. Email lives in the Footer.
...
export function Contact() {
  ...
  const hasWhatsApp = Boolean(brandConfig.whatsappNumber?.trim());
  ...
  const showBox = hasWhatsApp || hasSocial;

  return (
    <section id="contacto" aria-labelledby="contacto-heading" className="py-12 sm:py-20 bg-brand-canvas">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-8 sm:mb-12">
          ...
          <h2 id="contacto-heading" className="text-2xl sm:text-4xl font-bold text-brand-ink mt-2 mb-3 sm:mb-4">
            {t.contact.headline}
          </h2>
        ...
        {/* Single consolidated box: WhatsApp CTA + gold divider + social follow */}
        {showBox && (
          <div
            className="rounded-xl sm:rounded-2xl border border-night-border overflow-hidden"
            style={{ background: NIGHTFALL_GRADIENT }}
          >
            {/* WhatsApp CTA */}
            {hasWhatsApp && (
              <div className="p-5 sm:p-8 md:p-12">
                <div className="flex flex-col md:flex-row items-center justify-between gap-5 sm:gap-8">
                  ...
                  >
                    <WhatsAppIcon className="w-6 sm:w-7 h-6 sm:h-7" />
                    {t.contact.startChat}
                  </a>
                </div>
              </div>
            ...
            {/* Gold divider: thin at ends, thick in middle; inset so it doesn't touch box edges */}
            {hasWhatsApp && hasSocial && (
              <div className="px-6 sm:px-10 md:px-16 py-0" aria-hidden>
                <div
                  className="h-1 rounded-full max-w-full"
                  style={{
                    background:
                      "linear-gradient(90deg, transparent 0%, rgba(248,207,28,0.3) 12%, #f8cf1c 25%, #f8cf1c 75%, rgba(248,207,28,0.3) 88%, transparent 100%)",
                  }}
                />
              </div>
            )}

            {/* Social follow (Instagram / TikTok) */}
            {hasSocial && (
              <div id="redes" className="p-5 sm:p-8 md:p-12" aria-labelledby="redes-heading">
                <div className="flex flex-col md:flex-row items-center justify-between gap-5 sm:gap-8">
                  <div className="text-center md:text-left">
                    <span className="text-brand-sky text-xs sm:text-sm font-semibold uppercase tracking-wider">
                      {t.social.sectionLabel}
                    </span>
                    <h3 id="redes-heading" className="text-xl sm:text-2xl font-bold text-night-text mt-1 mb-1 sm:mb-2">
                      {t.social.headline}
                    </h3>
                    <p className="text-night-muted text-sm sm:text-base">
                      {t.social.subtext}
                  ...
                  <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full md:w-auto">
                    {hasInstagram && (
                      ...
                      >
                        <InstagramIcon className="w-6 sm:w-7 h-6 sm:h-7" />
                    ...
                    {hasTikTok && (
                      ...
                      >
                        <TikTokIcon className="w-6 sm:w-7 h-6 sm:h-7" />
